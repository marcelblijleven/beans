...
def setup_one_coffee():
    # TODO: merge with user_with_one_coffee
...
def user_with_one_coffee(db, django_user_model) -> User:
    user = UserFactory.create(email="test@email.com")
    processing = ProcessingFactory.create(user=user, name="Natural")
    roaster = RoasterFactory.create(user=user, name="La Cabra")
...
def test_data_dir():
    ...
    file_path = os.path.abspath(os.path.join(dir_path, "data"))
    return file_path
