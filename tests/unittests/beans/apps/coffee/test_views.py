...
@pytest.mark.django_db
def test_coffee_list_view(client, rf, django_user_model, mocker: MockFixture):
    ...
    assert mock_render.mock_calls[0].kwargs["context"]["coffee_list"].count() == user.coffee_set.count()
...
@pytest.mark.django_db
def test_coffee_detail_view_does_not_exist_for_user(rf, user_with_one_coffee, secondary_user_with_one_coffee, mocker: MockFixture):
    mock_detail_information = mocker.patch(
        "beans.apps.coffee.views.get_detail_information",
        wraps=get_detail_information,
    )
